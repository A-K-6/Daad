...
describe('UpdateService', () => {
  ...
  it('returns current version', () => {
    expect(updateService.getCurrentVersion()).toBe('0.1.2');
  });
  ...
  it('detects when an update is available from GitHub API', async () => {
    const mockRelease = {
      ...
      assets: [{ browser_download_url: 'https://github.com/A-K-6/Daad/releases/download/v0.2.0/Daad.dmg' }],
    };
  ...
  it('detects when app is up-to-date', async () => {
    const mockRelease = {
      ...
      body: 'Latest release',
      published_at: '2026-09-02T00:00:00Z',
      html_url: 'https://github.com/A-K-6/Daad/releases/tag/v0.1.2',
      assets: [],
    };
